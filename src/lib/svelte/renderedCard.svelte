...
<div class="relative overflow-hidden" style="width: {320 * scale}px; height: {400 * scale}px;">
	<div class="absolute top-0 left-0 origin-top-left" style="transform: scale({scale}); width: 320px; height: 400px;">
		
		
		
			<img src={cardVersion?.imageUrl} alt={squadName} class="pointer-events-none absolute -top-8" />
			...
			<div class="absolute top-[20%] left-[21%] -translate-x-1/2 text-center scale-125">
				<div class="scale-y-[1.2] transform text-[1.73em] leading-[0.91em] font-[800]">
					{card?.rating}
				</div>
				<div class="mt-0.5 scale-y-[1.2] transform text-[0.83em] font-bold">
					{card?.preferredPosition}
				</div>
			</div>

			<div class="absolute top-[63%] right-1/2 w-full translate-x-1/2 ">
				<div class="text-2xl font-black whitespace-nowrap mb-1">{card?.cardName}</div>
			...
			<div class="absolute top-[82%] flex w-full items-center justify-center gap-2 scale-125">
